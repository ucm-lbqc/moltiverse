...
OptionParser.parse do |parser|
  ...
  parser.on("-o NAME", "--output_name=NAME", "Output basename. Default: sdf_rmsd") do |str|
    output_name = str
  end
...
Dir["#{pdbs_path}/*.pdb"].each do |pdb|
  ...
  rmsd = 99999
  st_min = Chem::Structure.new
  pdb_min = Chem::Structure.new
  sdf_structures.map_with_index do |_, idx|
    ...
    if rmsd_tmp < rmsd
      rmsd = rmsd_tmp
      selection_st.coords.align_to(selection_pdb.coords)
      st_min = selection_st
    end
  end
  st_min.to_pdb("#{output_name}_#{basename}_min_rmsd_#{rmsd}.pdb", bonds: :none)
end
